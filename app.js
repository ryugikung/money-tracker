...
function renderTransactionList(container,list,empty='No transactions yet.'){
  ...
  container.innerHTML=list.map(t=>{const c=categoryById(t.category_id);const dt=new Date(t.occurred_at);return `<div class="transaction-row" data-tx="${t.id}"><div class="category-dot" style="background:${colorWithAlpha(c?.color||'#999')};color:${c?.color||'#777'}">${t.type==='income'?'↗':'↘'}</div><div class="transaction-main"><strong>${escapeHtml(c?.name||'Uncategorized')}</strong><small>${escapeHtml(t.note||'No note')} · ${dt.toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})} ${dt.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}</small></div><div class="transaction-amount ${t.type}">${t.type==='income'?'+':'−'}${money(t.amount)}</div></div>`}).join('');
...
function setTransactionType(type){ state.transactionType=type; document.querySelectorAll('#transactionTypeToggle button').forEach(b=>b.classList.toggle('active',b.dataset.type===type)); populateCategorySelect(); }
...
function renderCalendar(){
  ...
  for(let i=0;i<42;i++){const d=new Date(start);d.setDate(start.getDate()+i);const key=localDateInput(d);const dayTx=state.transactions.filter(t=>localDateInput(new Date(t.occurred_at))===key);const inc=dayTx.filter(t=>t.type==='income').reduce((s,t)=>s+Number(t.amount),0),exp=dayTx.filter(t=>t.type==='expense').reduce((s,t)=>s+Number(t.amount),0);html+=`<button class="calendar-day ${d.getMonth()!==m?'other':''} ${key===today?'today':''}" data-date="${key}"><div class="day-num">${d.getDate()}</div><div class="day-sums">${inc?`<span class="day-sum income">+${money(inc)}</span>`:''}${exp?`<span class="day-sum expense">−${money(exp)}</span>`:''}</div></button>`}
